...
work_dir = os.path.join(base_dir, 'evaluation_public.txt')
...
def work():
    ...
    for i in range(num_batch):   # 逐批次处理
        ...
        y_pred_cls[start_id:end_id] = session.run(model.y_pred_cls, feed_dict=feed_dict)
    count=0
    f_t1 = open("F:/BDCI2017-360/submit1.csv", 'w', encoding='utf-8')
    #f_t2 = open("F:/BDCI2017-360/submit2.csv", 'w', encoding='utf-8')
    with open("F:/BDCI2017-360/evaluation_public_id.txt", 'r', encoding='utf-8') as f1:
        for line in f1:
            line1=line.replace("\n","")
...
if __name__ == '__main__':
    if len(sys.argv) != 2 or sys.argv[1] not in ['train', 'test','work']:
